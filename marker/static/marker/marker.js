...
window.onload = function() {
  ...
  $(document).bind('keydown', "Shift+right",incrWidth);
  $(document).bind('keydown', "Shift+left",decrWidth);
  $(document).bind('keydown', "Shift+up",incrHeight);
  $(document).bind('keydown', "Shift+down",decrHeight);
  $(document).bind('keydown', "s",save);
  $(document).bind('keydown', "l",load);
  ...
  $(document).bind('keydown', "Shift+n",nextI);
  $(document).bind('keydown', "Shift+p",prevI);
...
function changeID() {
  console.log("[INFO] Change ID hello world")
...
function moveRect(msg,id) {
  ...
  for (var i = 0; i < msg.length; i++) {
    ...
    if(oldRect.y1 != 0) {
      ...
      var heightR = parseInt($("#pHeight").text());
      var widthR = parseInt($("#pWidth").text());
    }
    if(newRect.ratio > 0){
