...
def frame(request,workerID):
    ...
    try:
        ...
        print("settings.CAMS in frame view ", settings.CAMS)
...
def processFrame(request,workerID):
    print("[INFO] processFrame")
    context = RequestContext(request)
    try:
        w = Worker.objects.get(pk = workerID)
        if w.state == 1 and w.frame_labeled >= 12:
            #w.state = 2
            w.state = 1  
...
def updateWorker(workerID, state):
    print("[INFO] updateWorker")
    w = Worker.objects.get(pk = workerID)

def generate_code(worker):
    print("[INFO] generate_code")
    try:
...
def updateNumCam(request):
    print("[INFO] updateNumCam")
    context = RequestContext(request)
    if request.is_ajax():
        try:
            wID = request.POST['workerID']
            settings.NB_CAMS = int(request.POST['numCameras'])
            print('settings.NB_CAMS ', settings.NB_CAMS)
            settings.FIND_RECT = [[{} for _ in range(2913)] for _ in range(settings.NB_CAMS)]
            settings.RECT = [{} for _ in range(settings.NB_CAMS)]
            for line in settings.LINES[10:]:
                l = line.split()
                cam = int(l[1])
                id_rect = int(l[2])
                if l[3] != "notvisible":
                    ...
                    if d < 5000:
                        if abs(c - a) < abs(d - b):
                            if cam < settings.NB_CAMS:
                                settings.RECT[cam][id_rect] = (a, b, c, d,ratio)
                                settings.FIND_RECT[cam][d][(a + c) // 2] = id_rect
        
            settings.CAMS = []

            for i in range(settings.NB_CAMS):
                settings.CAMS.append("C" + str(i+1) + "/")
            
            return redirect("/gtm_hit/"+wID+"/processInit")
        except KeyError:
            return HttpResponse("Error")
    else:
        return HttpResponse("Error")
