...
var to_label = 13;
var num_camera_row = (cameras - (cameras % 3)) + 1; // Maximum 3 camera display in a row
var canv_width = 4; // divide col into 12 parts https://getbootstrap.com/docs/4.0/layout/grid/
...
window.onload = function() {
  ...
  console.log("[INFO] On load window")
  ...
  //   canv.width='1920';
  //   canv.height='1080';
  //   canv.title='Camera '+camid;
  //   canv.className="img col-sm-"+canv_width;
  //   document.getElementById('ul1').appendChild(canv); // adds the canvas to #someBox
  // }
  ...
  //     console.log("[INFO] Load camera " + this.id)
  //     var c = document.getElementById("canv"+this.id);

  //     if(c == null){
  //       console.log("[ERROR] Could not load camera "+this.id)
  //       alert("[ERROR] Could not load camera "+this.id);
  //     }
  ...
  $("#pHeight").val(-1);
  $("#pWidth").val(-1);
  $("#numCameras").bind( "keydown", "c",updateNumCams);
  $("#numCameras").val(cameras);
...
function load_cameras(){
  console.log(cams)
  camName = cams.substring(2,cams.length-2).split("', '");
  cameras = camName.length;
  // Add cameras canvas
  console.log("[INFO] Load cameras " + cameras)
  canv_width = 4;
  if(cameras==2)
    canv_width = 6;
  else if(cameras==1)
...
function changeFrame(order,increment) {
  ...
  $.ajax({
    ...
    success: function(msg) {
      ...
      //   button.text = "Finish";
      // }
      console.log("[INFO] Change frame Successfully")
...
function updateNumCams() {
  numCameras = parseInt($("#numCameras").val());
  cameras = numCameras
  num_camera_row = (cameras - (cameras % 3)) + 1;

  console.log("[INFO] Update number of cameras " + cameras)
  load_cameras();
  $.ajax({
    method: "POST",
    url: 'updateNumCam',
    ...
    success: function(msg) {
      console.log("[INFO] Update Cam Successfully")
    },
    complete: function() {
      // window.onload();
...
function drawRect() {
  console.log("[INFO] Draw rect")
  console.log("[INFO] cameras " + cameras)
  for (var i = 0; i < cameras; i++) {
